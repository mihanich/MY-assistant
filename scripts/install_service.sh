...
for service in systemd/*.service; do
	sed "s:/home/__USER__/squeezebox_assistant:${repo_path}:g;s:__USER__:${GIT_OWNER}:g" "$service" \
	 > "/lib/systemd/system/$(basename "$service")"
done

sudo systemctl enable lmsassistant.service
sudo systemctl start lmsassistant.service
